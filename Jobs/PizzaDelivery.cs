...
namespace LSRP_VFR.Jobs
{
    public class PizzaDelivery : Script
    {
        ...
        private void OnResourceStart()
        {
            ...
            API.setBlipName(myBlip, "Metier : Livreur Pizza");
            ...
            m_colShape.onEntityEnterColShape += (shape, entity) =>
            {
                ...
                else
                {
                    ...
                    Actions.Add("Fin de service");
                    Actions.Add("Quitter le menu");
                    API.triggerClientEvent(players, "bettermenuManager", 116, "Menu Livreur Pizza", "", false, Actions);
                }
        ...
        public void StartMission(Client player)
        {
            if(player.getData("LivreurPizza") == false || player.getData("LivreurPizza") == null)
            {
                ...
                API.sendNotificationToPlayer(player, "Montez dans un vehicule et rendez vous au differents markers");
                for (int i = 5; i > 0; i--)
                {
                    ...
                    if (i == 5)
                    {
                        API.sendNotificationToPlayer(player, "Vous avez " + i + "pizza a livrer, les points de livraison se trouve sur les markers rouge");
                    }
                    else
                    {
                        API.sendNotificationToPlayer(player, "Vous avez encore " + i + "pizza a livrer");
                    }
        ...
        {
            API.sendNotificationToPlayer(player,"Vous avez terminé vos livraisons, retournez au depôt pour recuperer votre paie.");
                ...
                else
                {
                    if (players.handle == player.handle)
                    {
                        if (player.isInVehicle)
                        {
                            ...
                            {
                                API.sendNotificationToPlayer(player, "Vous avez terminé vos livraisons");
                            ...
                            {
                                API.sendNotificationToPlayer(player, "Vous devez être dans un vehicule de votre metier");
                            }
                        }
                        else
                        {
